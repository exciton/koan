...
Legacy ``.koan-restart`` (DEPRECATED, S5): the single combined marker is no
...
# is DEPRECATED (S5): no longer written by ``request_restart``. The ``None``
...
def request_restart(koan_root: str) -> None:
    ...
    longer written (S5) — nothing reads it.
    """
