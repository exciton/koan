...
class TestCheckAndInject:
    ...
    def test_no_recurring_file(self, tmp_path):
        ...
        result = check_and_inject(recurring_path)
        assert result == []
    ...
    def test_injects_due_mission(self, tmp_path):
        ...
        with patch("app.utils.KOAN_ROOT", tmp_path):
            result = check_and_inject(recurring_path, now)
    ...
    def test_injects_with_project_tag(self, tmp_path):
        ...
        now = datetime(2026, 2, 3, 8, 0)
        with patch("app.utils.KOAN_ROOT", tmp_path):
            check_and_inject(recurring_path, now)
...
class TestForceRun:
    ...
    def test_force_run_single_by_number(self, tmp_path):
        ...
        with patch("app.utils.KOAN_ROOT", tmp_path):
            result = force_run(recurring_path, identifier="1", now=now)
    ...
    def test_force_run_single_by_keyword(self, tmp_path):
        ...
        now = datetime(2026, 2, 3, 10, 0)
        result = force_run(recurring_path, identifier="check", now=now)
    ...
    def test_force_run_all_enabled(self, tmp_path):
        ...
        now = datetime(2026, 2, 3, 10, 0)
        with patch("app.utils.KOAN_ROOT", tmp_path):
            result = force_run(recurring_path, identifier=None, now=now)
    ...
    def test_force_run_updates_last_run(self, tmp_path):
        ...
        now = datetime(2026, 2, 3, 10, 0)
        force_run(recurring_path, identifier="1", now=now)
    ...
    def test_force_run_empty_list(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="No recurring missions"):
            force_run(recurring_path, identifier="1")
    ...
    def test_force_run_invalid_identifier(self, tmp_path):
        ...
        with pytest.raises(ValueError):
            force_run(recurring_path, identifier="nonexistent")
